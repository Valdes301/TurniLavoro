...
import { Clock, AlertCircle, Sparkles, CheckCircle2, SlidersHorizontal, X, Save, Calculator, Store, Briefcase, Coffee, TrendingUp, Moon, Sun, Palmtree } from 'lucide-react';
import { WeekSummary, DeficitCoverageSplit, WeeklyDeficitCoverage } from '../types';
import { formatHours, formatOvertime, saveWeeklyDeficitCoverageSplit, getLocationBadge, isNightShift } from '../utils/shiftUtils';
...
export const WeeklyDeficitSelector: React.FC<WeeklyDeficitSelectorProps> = ({
  ...
  // Render Modal Dialog for easy split configuration
  const renderModal = () => {
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl max-w-lg w-full p-5 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-extrabold text-slate-900 dark:text-slate-100">
                  Ripartizione Ore in Negativo
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Settimana {week.weekIndex} ({week.weekLabel}) • Mancano -{formatHours(week.deficitHours)}
                </p>
              </div>
            </div>
            <button
          ...
          <div className="pt-2 flex justify-end gap-2">
            ...
            <button
              ...
              className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-2xs transition-colors cursor-pointer"
            >
              <Save className="w-4 h-4" />
              <span>Salva Ripartizione</span>
            </button>
          </div>
